...
exports.noFilterInQuery = void 0;
...
function unwrapExpression(expr) {
    ...
    while (true) {
        ...
        // ignore `await`
        if (current.type === utils_1.AST_NODE_TYPES.AwaitExpression) {
        ...
        if (current.type === utils_1.AST_NODE_TYPES.TSTypeAssertion) {
            current = current.expression;
            continue;
        }
        // Ignore generic arguments (e.g. `<T>` in `fn<T>()`)
        if (current.type === utils_1.AST_NODE_TYPES.TSInstantiationExpression) {
            current = current.expression;
            continue;
        }
        // Ignore parentheses
...
function isTerminalQueryCall(expr) {
...
function isDbQueryChainFallback(expr) {
    const unwrapped = unwrapExpression(expr);
    if (unwrapped.type !== utils_1.AST_NODE_TYPES.CallExpression)
        return false;
    const callee = unwrapped.callee;
    ...
    if (methodName === "query") {
        const baseObject = unwrapExpression(callee.object);
        // `ctx.db.query(...)`
        if (baseObject.type === utils_1.AST_NODE_TYPES.MemberExpression &&
            baseObject.property.type === utils_1.AST_NODE_TYPES.Identifier &&
            baseObject.property.name === "db") {
            return true;
        }
        // `db.query(...)` (DatabaseReader / DatabaseWriter passed around)
        if (baseObject.type === utils_1.AST_NODE_TYPES.Identifier &&
            baseObject.name === "db") {
            return true;
        }
        ...
        if (baseObject.type === utils_1.AST_NODE_TYPES.MemberExpression &&
            ...
            const innerObject = unwrapExpression(baseObject.object);
            if (innerObject.type === utils_1.AST_NODE_TYPES.Identifier &&
                innerObject.name === "db") {
                return true;
            }
            if (innerObject.type === utils_1.AST_NODE_TYPES.MemberExpression &&
                innerObject.property.type === utils_1.AST_NODE_TYPES.Identifier &&
    ...
    return isDbQueryChainFallback(callee.object);
}
exports.noFilterInQuery = (0, util_js_1.createRule)({
    name: "no-filter-in-query",
    meta: {
        type: "suggestion",
        docs: {
            description: "Warn when using `.filter()` on a Convex database query, since it can be inefficient.",
        },
        messages: {
            "no-filter-in-query": "Avoid calling `.filter()` on a Convex database query; it can be inefficient (see Convex query best practices). If you have a real reason to use it, disable with `// eslint-disable-next-line @convex-dev/no-filter-in-query`.",
        },
        schema: [],
    },
    defaultOptions: [],
    create: (context) => {
        ...
        function typeContainsSymbolNamed(type, name) {
        ...
        function isConvexQueryFilterCall(node) {
            ...
            try {
                ...
                if (params.length < 1)
                    return false;
                const predicateParamType = checker.getTypeOfSymbolAtLocation(params[0], tsCall);
                const predicateSigs = predicateParamType.getCallSignatures();
                if (predicateSigs.length === 0)
                    return false;
                const predSig = predicateSigs[0];
                const predParams = predSig.getParameters();
                if (predParams.length < 1)
                    return false;
                const qParamType = checker.getTypeOfSymbolAtLocation(predParams[0], tsCall);
        ...
        const queryVarSymbols = new Set();
        const queryBuilderTypes = [];
        const queryBuilderTypeStrings = new Set();
        function addQueryBuilderType(t) {
            if (!checker)
                return;
            const s = checker.typeToString(t);
            if (s === "any" || s === "unknown")
                return;
            if (queryBuilderTypeStrings.has(s))
                return;
        ...
        function isConvexQueryBuilderType(t) {
            if (!checker)
                return false;
        ...
        function isIdentifierExplicitAnyType(id) {
            return (id.typeAnnotation?.typeAnnotation.type === utils_1.AST_NODE_TYPES.TSAnyKeyword);
        }
        function getCallReturnTypes(fnType) {
            if (!checker)
                return [];
        ...
        function getPropertyType(symbol, fallbackNode) {
            if (!checker) {
                // Should never happen if we call this.
                return {};
            }
            const decl = symbol.valueDeclaration ?? symbol.declarations?.[0] ?? fallbackNode;
            return checker.getTypeOfSymbolAtLocation(symbol, decl);
        }
        function seedQueryBuilderTypesFromGeneratedServer() {
            if (!checker || !services?.program)
                return;
            let anyDatabaseReader = null;
            let generatedServerSourceFile = null;
            try {
            ...
            if (!anyDatabaseReader || !generatedServerSourceFile)
                return;
        ...
        return {
            CallExpression(node) {
                ...
                if (checker &&
                    ...
                    const initExpr = varDecl.init;
                    if (initExpr && isDbQueryChainFallback(initExpr)) {
                        try {
                            const idTsNode = tsNodeMap.get(varDecl.id);
                            const sym = checker.getSymbolAtLocation(idTsNode);
                            if (sym &&
                                !isIdentifierExplicitAnyType(varDecl.id)) {
                                queryVarSymbols.add(sym);
                            }
                            const initTsNode = tsNodeMap.get(unwrapExpression(initExpr));
                            addQueryBuilderType(checker.getTypeAtLocation(initTsNode));
                        }
                        catch {
                            // ignore
                        }
                ...
                if (hasTypeInfo) {
                    try {
                        ...
                        const hasUsefulReceiverType = receiverTypeString !== "any" && receiverTypeString !== "unknown";
                        if (receiverUnwrapped.type === utils_1.AST_NODE_TYPES.Identifier) {
                            const sym = checker.getSymbolAtLocation(receiverTsNode);
...
//# sourceMappingURL=no-filter-in-query.js.map